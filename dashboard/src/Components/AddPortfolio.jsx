...
function AddPortfolio() {
  ...
  const FetchData = async () => {
    ...
    await axios
      .post("http://localhost:8000/api/team/portfolio", formData).then((res) => {setName(res);})
          ...
          <div className="slide-form">
            <div className="slideinput">
              <form encType="multipart/form-data">
                <div className="inp-cont"><label className="form-lable">Name</label>
                  <input type="text"name="name"className="form-control"value={name}onChange={(e) => setName(e.target.value)}/>
                </div>
                <div className="inp-cont">
                  <input type="file"name="slideImage"accept="image/png,image/jpg, image/jpeg"onChange={handleImgChange} />
                </div>
                <div className="inp-cont "><button className="btn btn-success" onClick={handleAddPort}>Submit</button>
                </div>
              </form>
            </div>
          </div>
